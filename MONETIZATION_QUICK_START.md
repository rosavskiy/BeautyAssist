# 🎯 Quick Start - Система монетизации

## Для тестирования

### 1. Настройка не требуется
Все работает через Telegram Stars из коробки!

### 2. Запустить бота
```bash
python -m bot.main
```

### 3. Протестировать подписки
```
/subscription - просмотр статуса
🎁 Активировать trial - получить 14 дней бесплатно
💎 Выбрать тариф - купить подписку через Telegram Stars
```

### 4. Создать промокод (через БД)
```sql
INSERT INTO promo_codes (code, type, discount_percent, status, valid_from, max_uses, max_uses_per_user)
VALUES ('NEWYEAR2025', 'percent', 20, 'active', NOW(), 100, 1);
```

### 5. Проверить уведомления
Background task проверяет подписки каждые 6 часов автоматически.

---

## Реализованные фичи

✅ **Подписки**:
- Trial 14 дней
- Monthly 990₽
- Quarterly 2490₽ (скидка 15%)
- Yearly 8280₽ (скидка 30%)

✅ **Промокоды**:
- Процентные скидки
- Фиксированные скидки  
- Продление trial
- Лимиты использования
- Реферальная система

✅ **Уведомления**:
- За 3 дня до истечения
- За 1 день до истечения
- При истечении подписки

✅ **Платежи**:
- Telegram Stars ✅

---

## Admin функции

### Промокоды (будет в админке)
```python
# Создать промокод
code = await promo_repo.create_promo_code(
    code="NEWYEAR2025",
    type=PromoCodeType.PERCENT,
    discount_percent=20,
    max_uses=100,
)

# Статистика
stats = await promo_repo.get_promo_code_stats("NEWYEAR2025")
# {"usage_count": 45, "total_discount_given": 8910}
```

### Финансовые метрики (будет в админке)
```python
# MRR, конверсия, churn
metrics = await admin_repo.get_subscription_metrics()
```

---

## Структура проекта

```
database/
├── models/
│   ├── subscription.py      # Subscription, SubscriptionStatus
│   ├── transaction.py        # Transaction, TransactionStatus
│   └── promo_code.py         # PromoCode, PromoCodeUsage
├── repositories/
│   ├── subscription.py       # SubscriptionRepository
│   └── promo_code.py         # PromoCodeRepository
services/
├── payment.py                # PaymentService (Telegram Stars)
└── subscription_monitor.py   # Уведомления об истечении
bot/
├── handlers/
│   └── subscription.py       # /subscription handlers
├── middlewares/
│   └── subscription.py       # SubscriptionMiddleware
└── subscription_plans.py     # Конфигурация тарифов
```

---

## Следующие шаги

1. **Админка**: UI для управления промокодами
2. **Метрики**: MRR, конверсия, churn в дашборде
3. **UI**: Применение промокодов при покупке

Система готова к использованию! 🚀
